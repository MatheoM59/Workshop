...
//   // Gestion du changement de statut sanitaire
//   const handleStatusChange = async (e: React.FormEvent) => {
//     e.preventDefault();
//     if (!userId) return;

//     try {
//       await marquerTraite(selectedStatus, Number(userId));
//       setFeedback(` Statut de l'agent #${userId} mis à jour : ${selectedStatus}`);
//       setUserId("");
//     } catch (error) {
...
//   // Simulation du passage de portail NFC
//   const handleGatePass = async (e: React.FormEvent) => {
//     e.preventDefault();
//     if (!userId || !targetRoom) return;

//     // Simulation de la règle métier : blocage si quarantaine
//     if (selectedStatus === "QUARANTINE") {
//       setFeedback(` ACCÈS REFUSÉ : L'agent #${userId} est en QUARANTINE. Portes verrouillées.`);
//       return;
//     }

//     try {
//       // On réutilise marquerTraite ou une mise à jour directe si besoin
//       setFeedback(` ACCÈS AUTORISÉ : L'agent #${userId} a franchi le portail vers la zone ${targetRoom}.`);
//       setTargetRoom("");
//     } catch (error) {
//       setFeedback(" Erreur lors de la simulation du passage.");
//     }
//   };

//   return (
//     <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
//       {/* 1. Modification du statut sanitaire */}
...
//   // Auto-sélection au chargement des props BDD
//   useEffect(() => {
...
//       setFeedback(` Statut de ${selectedUser.firstname} ${selectedUser.lastname} (#${selectedUser.id}) mis à jour : ${selectedStatus}`);
//     } catch (error) {
//       setFeedback(" Erreur lors de la mise à jour du statut.");
//     }
//   };
...
//     const isQuarantineSector = selectedSector.trim().toLowerCase().startsWith("secteur q") || 
//                                selectedSector.trim().toLowerCase().startsWith("q");
...
//     // Règle 2 : Un agent sain ne peut PAS accéder aux secteurs de Quarantaine
//     if (selectedUser.health_status !== "QUARANTINE" && isQuarantineSector) {
//       setFeedback(
...
//   return (
//     <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
//       {/* 1. Formulaire de changement de statut */}
//       <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
//         <h3 className="mb-4 text-base font-semibold text-slate-900">
//           Changement de Statut Sanitaire
//         </h3>
...
//                 <div className="mb-2 flex items-center gap-2 border-b border-slate-100 pb-2 px-1">
...
//               >
//                 {rooms.length === 0 ? (
//                   <option value="">Aucune room en BDD</option>
//                 ) : (
//                   rooms.map((room) => (
...
export const Simulation: React.FC<SimulationProps> = ({
  ...
  const handleStatusChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mise à jour synchrone de l'objet en mémoire
      setSelectedUser((prev) =>
        prev ? { ...prev, health_status: selectedStatus } : null
      );
...
const handleGatePass = async (e: React.FormEvent) => {
    ...
    if (selectedUser.health_status === "QUARANTINE" && !isQuarantineSector) {
      ...
      await enregistrerPassageGate(selectedUser.id, selectedSector, selectedRoomId, false);
    ...
    await enregistrerPassageGate(selectedUser.id, selectedSector, selectedRoomId, true);
